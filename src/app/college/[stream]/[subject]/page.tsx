import { getSubjectTopics, getAllCollegeParams } from '@/lib/mdx';
...
export async function generateStaticParams() {
  const allParams = await getAllCollegeParams();
  
  const unique = new Set<string>();
  const params: { stream: string; subject: string }[] = [];
  
  for (const p of allParams) {
    const key = `${p.stream}-${p.subject}`;
    if (!unique.has(key)) {
      unique.add(key);
      params.push({ stream: p.stream, subject: p.subject });
    }
  }
  return params;
}
...
export async function generateMetadata({
  ...
  return {
    title: `${subject.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} | ${stream.replace(/-/g, ' ').toUpperCase()}`,
    description: `Study materials and notes for ${stream.replace(/-/g, ' ')} ${subject.replace(/-/g, ' ')}.`,
  };
}
...
export default async function CollegeSubjectPage({
  ...
  const { stream, subject } = await params;
  const groups = await getSubjectTopics(['college', stream, subject]);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-[calc(100vh-var(--header-height))]">
      ...
      <div className="mb-10">
        ...
        <p style={{ color: 'var(--text-secondary)' }} className="capitalize">
          {stream.replace(/-/g, ' ')} curriculum notes and resources.
        </p>
      </div>
